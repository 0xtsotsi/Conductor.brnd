 ...
 * After successful login, stores JWT and redirects to dashboard.
 *
 ...
 * - Keycloak
 *
 * Also supports dev mode with mock login for local development.
 */
export function LoginPage() {
  ...
  const handleOAuthLogin = async (provider: 'github' | 'google' | 'keycloak') => {
    ...
    try {
      ...
      // The provider will redirect back to /auth/callback with the token
      const authUrl = `${API_URL}/auth/${provider}?redirect=${encodeURIComponent(
    ...
    <div className="min-h-screen bg-mastra-bg-1 flex items-center justify-center px-4">
      <div className="max-w-md w-full">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-mastra-el-text mb-2">
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full px-4 py-3 bg-mastra-el-3 hover:bg-mastra-el-4 text-mastra-el-text rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3"
          >
          ...
          {DEV_MODE && (
            <button
              ...
              disabled={isLoading}
              className="w-full px-4 py-3 bg-mastra-el-accent hover:bg-mastra-el-accent-hover text-white rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Dev Login (Admin)
            </button>
          )}
        ...
        {isLoading && (
          <div className="mt-6 text-center">
            <p className="text-sm text-mastra-el-text-muted">Signing in...</p>
          </div>
        )}
